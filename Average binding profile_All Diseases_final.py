...
if median_norm:
    ...
    filename1 = 'sequence_data_NIBIB_DENV_CTS_seraCare_CV3_noMed18-Dec-2020-15-34.csv'
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[0], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[1], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[2], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[3], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[4], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
for i in range(0, data_tem.shape[1]):
    if log_scale:
        sns.distplot(np.log10(data_tem.iloc[:, i]), norm_hist=True, color=colors[5], hist=False,
                     kde_kws=dict(linewidth=0.2))
        plt.xlabel('log10(Binding intensity)', fontsize=15)
    else:
...
## this is for binding distribution of serum samples from two different sources
filename1 = 'sequence_data_NIBIB_DENV_CTS_seraCare_CV3_noMed18-Dec-2020-15-34.imporfdslkfdkfdfcsv'
data = pd.read_csv(filename1, header=None)
CTS_data = data.iloc[:, 1:27]  # parse CTS serum samples
SeraCare_data = data.iloc[:, 27:]  # parse SeraCare serum samples
data_mean = np.mean(data.iloc[:, 1:], axis=1)
data_mean = data.mean.reshape(len(data_mean), 1)
colors = ['b', 'violet']
color_mean = 'purple'

fig1 = plt.figure()
for i in range(CTS_data.shape[1]):
    sns.distplot(np.log10(CTS_data.iloc[:, i]), norm_hist=True, color=colors[0], hist=False,
                 kde_kws=dict(linewidth=0.2))
for i in range(SeraCare_data.shape[1]):
    sns.distplot(np.log10(SeraCare_data.iloc[:, i]), norm_hist=True, color=colors[1], hist=False,
                 kde_kws=dict(linewidth=0.2))
plt.xlabel('log10(Binding intensity)', fontsize=15)
plt.ylabel('Normalized density', fontsize=15)
plt.title('Binding distribution of DENV4 serum samples', fontsize=17)
# add color legends to the plot
import matplotlib.patches as patches
CTS_patch = patches.Patch(color=colors[0], label='CTS')
SeraCare_patch = patches.Patch(color=colors[1], label='SeraCare')
plt.legend(handles=[CTS_patch, SeraCare_patch])
