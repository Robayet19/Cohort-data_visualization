...
case_name = ['DENV4', 'Uninfected', 'WNV', 'HCV', 'HBV', 'Chagas']
...
def density_calculation(data_set):
    ...
    mean_contrast = mean_contrast_all[data_set][0]
    pvalues = file_run[data_set][0]
    data = np.vstack([mean_contrast, pvalues])  # combine the mean differences with p-values
...
for i in range(len(mean_contrast_all)):
    ...
    plt.xticks(np.arange(-1.4, 1.4 + 0.2, 0.2))
